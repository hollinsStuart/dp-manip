...
- [x] RGB 数据链路冒烟验证：6 个任务「生成 → rgb/state 转换 → 修正第 0 帧 → 导出 → VariDP 训练与评估」全部跑通（9.25，见 [docs/0925-smoke.md](./docs/0925-smoke.md)）。
...
- [x] `dp_manip` 支持 RGB 训练与评估（9.26）：`task.obs_mode = "rgb"` 读 `obs_rgb/{rgb,state}`，按官方 `train_rgbd.py` 用 PlainConv 编码器（`dp_manip/obs_encoder.py`，只依赖 torch，可直接拷给队员的代码）。动作维数从数据读出，4 维 / 8 维都能用。六个任务的配置在 `configs/rgb/`。Mac 上的离线检查全部通过；state 路径与改动前逐位一致。Mac 不能做 RGB 评估（MoltenVK 在场景重建后渲染错误，ubuntu 上 NVIDIA 与 lavapipe 都正常），见 `dp_manip/README.md`。
- [ ] wsl 上的 RGB 冒烟测试（9.26 wsl 连不上，还没做）：`check_dp_offline.py --device cuda`，`train_dp.py` 短跑并带验证，测 lavapipe 下每回合评估耗时与训练 ms/步，决定正式评估放 wsl 还是集群。
- [ ] 把 RGB 改动迁移到 VariDP / 7606-train-template：拷 `obs_encoder.py`，改数据读取（图像只存一份、窗口存索引），8 维任务补动作归一化并去掉执行前的 [-1, 1] 裁剪，评估环境换成 rgb。
