...
- [ ] **（必做，占 20 分）** 选定并完成一项受控研究实验（数据量、数据质量、超参数或架构）；可以考虑使用 25 / 50 / 100 / 200 条示范构造嵌套子集，研究数据效率。尽早定题，因为它决定每个任务要采多少条数据。（9.23：PickCube 10/25/50/100 条各 1 个训练种子已完成，final 0.02/0.14/0.47/0.77；写报告前需补训练种子。）
