...
#   REPOS="varidp" scripts/smoke/setup_teammates.sh   # only some of them (default: both)
set -euo pipefail
...
REPOS=${REPOS:-template varidp}
REPO=$(cd "$(dirname "$0")/../.." && pwd)
mkdir -p "$ROOT" "$REPO/logs"
exec > >(tee -a "$REPO/logs/smoke0925_setup.log") 2>&1
echo "=== setup_teammates $(date -Is) ROOT=$ROOT REPOS=$REPOS"
...
for name in $REPOS; do
  case "$name" in
    ...
    varidp)
      ...
      ;;
    *) echo "unknown repo: $name (template, varidp)" >&2; exit 1 ;;
  esac
done
echo "=== done $(date -Is)"
